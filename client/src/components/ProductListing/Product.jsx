...
const Product = ({
    ...
    // Handle both naming conventions from backend and ensure proper types
    const productId = _id || id;
    const productRating = Number(ratings ?? rating ?? 0);
    const reviewCount = Number(numOfReviews ?? ratingCount ?? 0);
    ...
    const updateWishlistUI = (add) => {
        setWishlistItems((prev) =>
            add ? [...prev, productId] : prev.filter((item) => item !== productId)
        );
    };
